...
;;
;;                 student LiU-ID: samer177
;; + lab group participant LiU-ID: davwi279
...
main:
	bl inituart
	bl initGPIOB
	bl initGPIOD				;initiera portar och avbrott
	bl initGPIOE
	bl initGPIOF
	bl initint
	CPSIE I

	mov r5,#(0x20001100 & 0xffff)		;addressen till multiplex-
	movt r5,#(0x20001100 >> 16)        	;ränkaren

	mov r4,#(0x20001000 & 0xffff)		;hämta addressen till klockans
	movt r4,#(0x20001000 >> 16)			;värde
...
power_saving:
	WFI									;gå in i strömsparläge när
	b power_saving						;avbrott ej sker
...
;*
intgpiod:
	push {r4,r5,r6,r7}						;pushar register som används utanför rutinen

	mov r0,#(GPIOD_GPIOICR & 0xffff)       	;MUX
	movt r0,#(GPIOD_GPIOICR >> 16)
	ldr r1,[r0]
	orr r1,r1,#0x80							;nollställ avbrott
	str r1,[r0]

	mov r5,#(0x20001100 & 0xffff)
	movt r5,#(0x20001100 >> 16)        	;hämta räknarvärdet
	ldr r6,[r5]

	mov r7,#(0x20001004 & 0xffff)
	movt r7,#(0x20001004 >> 16)

	cmp r6,r7							;kolla multiplex-ränkaren
	beq reset_counter

display_num:
	mov r0,#(GPIOF_GPIODATA & 0xffff)			;lägg in position
	movt r0,#(GPIOF_GPIODATA >> 16)

	strb r6,[r0]			;sätt in positionen på port F

	ldr r2,[r6]				;hämta siffran som är på positionen

	AND r3,r2,#0x0F
	ADR r4,SJUSEGTAB
	ADD r4,r4,r3
	LDRB r3,[r4]			;siffermönster läggs i r3

	mov r1,#(GPIOB_GPIODATA & 0xffff)
	movt r1,#(GPIOB_GPIODATA >> 16)

	str r3,[r1]				;lägg in siffran på port B

	add r6,r6,#0x1			;räkna upp multiplex-räknaren
	str r6,[r5]
...
reset_counter:
	mov r6,#(0x20001000 & 0xffff)		;nollställ multiplex-
	movt r6,#(0x20001000 >> 16)			;räknaren
...
intgpioe:
	push {r4,r5,r6,r7}						;pushar register som används utanför rutinen
	mov r3,#(GPIOE_GPIOICR & 0xffff)               ; BCD
	movt r3,#(GPIOE_GPIOICR >> 16)
	ldr r4,[r3]
	orr r4,r4,#0x10							;nollställ avbrott
	str r4,[r3]

	mov r0,#(0x20001000 & 0xffff)			;hämta sekund pos1
	movt r0,#(0x20001000 >> 16)				
	ldrb r1,[r0]							

	mov r8,#(0x20001001 & 0xffff)			;hämta sekund pos2
	movt r8,#(0x20001001 >> 16)
	ldrb r2,[r8]

	mov r9,#(0x20001002 & 0xffff)			;hämta minut pos1
	movt r9,#(0x20001002 >> 16)
	ldrb r3,[r9]

	mov r10,#(0x20001003 & 0xffff)			;hämta minut pos2
	movt r10,#(0x20001003 >> 16)
	ldrb r4,[r10]
	
	cmp r1,#9							;kolla på sekunden på pos1 är 9
	beq add_second_pos2
	add r1,r1,#0x1						;räkna upp sekunden på pos1
	strb r1,[r0]
...
add_second_pos2:
	mov r1,#0				;nollställ sekundräknare pos1
	strb r1,[r0]			;lägg in den på pos1
	
	cmp r2,#5
	beq add_minute_pos1
	
	add r2,r2,#1				;räkna upp och lägg i minnet
	strb r2,[r8]
...
add_minute_pos1:							
	strb r1,[r8]			;nollställ sekund pos2

	cmp r3,#0x9				;kolla om det ska räknas upp på position 2
	beq add_minute_pos2
	add r3,r3,#0x1				;räkna upp minuten på position 1
	strb r3,[r9]
...
add_minute_pos2:
	strb r1,[r9]			;nollställ minut pos1
	
	add r4,r4,#0x1				;räkna upp minut på position 2
	cmp r4,#0x6					;kolla om en timme har gått
	beq reset_clock
...
reset_clock:
	strb r1,[r10]					;nollställ minut pos2

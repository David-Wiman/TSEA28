...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
setpassword:
;	spara den första giltiga koden i minnet
...
; Förberedelseuppgift: Skriv denna subrutin!
	...
	bl clearinputwhileopen		; rensar bufferten där den potentiallt nya koden sparas

	b getkeywhileopen		; läs av tangentbordet i öppet läge
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
clearinputwhileopen:
	; rensar bufferten där den potentiallt nya koden sparas
	; 0x20001020-23 & 0x20001030-33
	mov r1,#(0x20001020 & 0xffff)
	movt r1,#(0x20001020 >> 16)
	mov r0,#0xFF
	str r0,[r1]

	mov r1,#(0x20001021 & 0xffff)
	movt r1,#(0x20001021 >> 16)
	mov r0,#0xFF
	str r0,[r1]

	mov r1,#(0x20001022 & 0xffff)
	movt r1,#(0x20001022 >> 16)
	mov r0,#0xFF
	str r0,[r1]

	mov r1,#(0x20001023 & 0xffff)
	movt r1,#(0x20001023 >> 16)
	mov r0,#0xFF
	str r0,[r1]

	mov r1,#(0x20001030 & 0xffff)
	movt r1,#(0x20001030 >> 16)
	mov r0,#0xFF
	str r0,[r1]

	mov r1,#(0x20001031 & 0xffff)
	movt r1,#(0x20001031 >> 16)
	mov r0,#0xFF
	str r0,[r1]

	mov r1,#(0x20001032 & 0xffff)
	movt r1,#(0x20001032 >> 16)
	mov r0,#0xFF
	str r0,[r1]

	mov r1,#(0x20001033 & 0xffff)
	movt r1,#(0x20001033 >> 16)
	mov r0,#0xFE
	str r0,[r1]
...
updatekeywhileopen:							; spara tangenttryck i r4
	...
	cmp r4,#0xC				; kontrollerar om samma kod tryckts in två gånger  i rad, när C trycks ned
	beq checkcodewhileopen

	cmp r4,#0xA			; om en bokstav förutom C trycktes in, lås och börja om
	bpl start

releasekeywhileopen:						; vänta på att tangenten släpps upp
	mov r8,#(GPIOE_GPIODATA & 0xffff)
	movt r8,#(GPIOE_GPIODATA >> 16)
	ldr r7,[r8]
	ands r7,#0x10
	bne releasekeywhileopen

	cmp r11,#0x8			; nollställ räknaren efter 8 tryck
	beq resetcounter

afterreset:
	cmp r11,#0x4		; avgör om tangenten hamnar i buffer 1 eller 2
	add r11,r11,#0x1		; räknar upp räknaren
	bmi addkeywhileopen1	; lägg till tangent i buffer 1
	b addkeywhileopen2		; eller buffer 2 om räknaren är tillräckligt hög
...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
addkeywhileopen2:
	; lägger nya koden i en annan buffert
	mov r1,#(0x20001030 & 0xffff)
	movt r1,#(0x20001030 >> 16)
	ldr r2,[r1]
	lsl r2,r2,#0x8
...
; Utargument: Inga
checkcodewhileopen:
; kontrollerar om samma kod tryckts in två gånger i rad
; om det är fallet så byts den giltiga koden

	mov r11,#0x0

	mov r1,#(0x20001020 & 0xffff)		; läs in första fyra tecknen kod
	movt r1,#(0x20001020 >> 16)
	ldr r0,[r1]

	mov r1,#(0x20001030 & 0xffff)		; läs in tecken 5-8
	movt r1,#(0x20001030 >> 16)
	ldr r2,[r1]

	cmp r0,r2				; jämför om de två koderna är samma
	bne getkeywhileopen

	mov r3,#(0x20001010 & 0xffff)		; sparar nya koden
...
printstring:
; Förberedelseuppgift: Skriv denna subrutin!
; skriv ut meddelandet med printchar, kontrollera att det är rätt antal tecken
	ldrb r0, [r4]
	push {lr}
	bl printchar
	pop {lr}

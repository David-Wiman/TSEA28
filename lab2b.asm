...
main:
	...
	mov r2,#(GPIOB_GPIODATA & 0xffff)		;bollens läge
	movt r2,#(GPIOB_GPIODATA >> 16)

	mov r9,#(0x20001100 & 0xffff)			;riktning
	movt r9,#(0x20001100 >> 16)

	mov r0,#(0x20001000 & 0xffff)				;hämta serveläge
	movt r0,#(0x20001000 >> 16)

	mov r4,#(0x20001200 & 0xffff)				;nollställ poäng höger
	movt r4,#(0x20001200 >> 16)

	mov r5,#(0x20001300 & 0xffff)				;nollställ poäng vänster
	movt r5,#(0x20001300 >> 16)


	mov r7,#0x0
	str r7,[r9]					;vänster spelare börjar
	str r7,[r4]
	str r7,[r5]
	mov r7,#0x80
	str r7,[r2]
	mov r7,#0xff				;sätt serve
	str r7,[r0]
...
travel:
	ldr r3,[r2]				;ta fram bollens läge

	ldr r5,[r9]				;ta fram bollens riktning

	cmp r5,#0x0				;kolla bollens riktning
	beq travel_left
	b travel_right
...
point_left1:
	mov r5,#(0x20001300 & 0xffff)
	movt r5,#(0x20001300 >> 16)
	ldr r6,[r5]
	add r6,r6,#0x1				;Öka vänster spelares poäng med 1
	str r6,[r5]

	mov r7,#0x80
	str r7,[r2]					;Ge bollen till vänster spelare
	mov r8,#0xff
	str r8,[r9]					;sätt riktning vänster
	str r8,[r0]					;sätt till serve
...
point_right1:
	mov r5,#(0x20001200 & 0xffff)
	movt r5,#(0x20001200 >> 16)
	ldr r6,[r5]
	add r6,r6,#0x1				;öka höger spelares poäng med 1
	str r6,[r5]

	mov r7,#0x1
	str r7,[r2]					;ge bollen till höger spelare
	mov r7,#0x0
	str r7,[r9]					;sätt riktning vänster
	mov r7,#0xff
	str r7,[r0]					;sätt till serve
...
;*
intgpiod:
	mov r10,#(GPIOD_GPIOICR & 0xffff)               ; Here is the interrupt routine triggered by port D
	movt r10,#(GPIOD_GPIOICR >> 16)
	ldr r6,[r10]
	orr r6,r6,#0x80							;nollställ avbrott
	str r6,[r10]
	...
	mov r9,#(0x20001100 & 0xffff)			; hämta riktning
	movt r9,#(0x20001100 >> 16)
	...
	mov r1,#0x0
	str r1,[r0] 				;sätt ej serve
	str r1,[r9]					;ändra riktning
...
;*
intgpioe:
  	mov r3,#(GPIOE_GPIOICR & 0xffff)               ; Here is the interrupt routine triggered by port E
